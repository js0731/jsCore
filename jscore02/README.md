...
前面提到，基本型別會有 **「屬性」** 以及 **「方法」** 是因為這些型別有著相對應的**包裹物件 (Primitive Wrapper)**，像下面段程式碼。
```
var str = 'Hello';  
...
這是因為在  `a = b.a = 1`這一行程式碼中，a會被賦予的值，是`b.a=1`這個**表達式所回傳的結果**，與`b.a`現在的值是**沒有關係的**。
可以在console中把`b.a`和`b.a=1`都印出來看看。
```
var b = {};  
...
**短路邏輯的運用**
  ...
  2. 用 && 來檢查物件與屬性值，如果沒有 `o` 這個物件就不會執行 o.getName()。
  `var name = o && o.getName();`
  
  3. 假設想要返回一個變數的`length`，但又不知道變數的類型可以使用 `if/else` 來檢查 `foo` 是否是一個可接受的類型。<br/>`return (foo | | [ ] ).length`
